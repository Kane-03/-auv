#!/usr/bin/env python3
"""
跟随者 PID 节点 (Robot 1 / Robot 2 / Robot 3 - Follower)
基于虚拟结构法 (Virtual Structure) 接管控制。
...
队形偏移（虚拟结构中心体坐标系，旋转到世界系后使用）:
  队形：圆上的三角形，旋转圆心为结构中心 (R=2.0)
  Robot 1: 0度     -> dx= 2.0, dy= 0.0
  Robot 2: 120度   -> dx=-1.0, dy= 1.732
  Robot 3: -120度  -> dx=-1.0, dy=-1.732
"""
import rospy
import numpy as np
...
class FollowerPIDNode:
    FORMATION_OFFSETS = {
        1: np.array([ 2.0,  0.0]),     # Robot 1：正前方
        2: np.array([-1.0,  1.732]),   # Robot 2：左后方
        3: np.array([-1.0, -1.732]),   # Robot 3：右后方
    }

    def __init__(self):
        rospy.init_node('follower_pid_node')
        self.robot_id    = rospy.get_param('~robot_id',    'bluerov2h_origin1')
        self.robot_index = int(rospy.get_param('~robot_index', 1))

        if self.robot_index not in self.FORMATION_OFFSETS:
            rospy.logerr(f'[Follower] 未知 robot_index={self.robot_index}，支持 1, 2, 或 3')
    ...
    def _init_ros(self):
        ...
        rospy.Subscriber('/formation/virtual_center_pose', PoseStamped,
                         self.center_pose_callback, queue_size=10)
        self.thruster_pubs = [
    ...
    def center_pose_callback(self, msg):        # 存储虚拟中心位姿
        self.center_pose     = [msg.pose.position.x,
                                msg.pose.position.y,
    ...
    def _compute_formation_target(self):
        """
        目标点 = 虚拟中心位置 + R(center_yaw) × 队形偏移
        偏移在虚拟中心体坐标系中定义，旋转到世界系后使用。
        """
        cx, cy, cyaw = self.center_pose     # 虚拟中心位姿（世界系）
        dx, dy = self.formation_offset      # 队形偏移（领航者体坐标系）
        cos_y  = np.cos(cyaw)               # 旋转矩阵元素
        sin_y  = np.sin(cyaw)               # 旋转矩阵元素
        world_dx = cos_y * dx - sin_y * dy  # 旋转偏移到世界系
        world_dy = sin_y * dx + cos_y * dy  # 旋转偏移到世界系
        return np.array([cx + world_dx, cy + world_dy, cyaw])   # 目标位姿（世界系）
    ...
    def control_step(self):
        ...
        if not self.center_received:
            rospy.logwarn_throttle(
                2.0, f'[Follower {self.robot_index}] 等待虚拟结构中心位姿...')
            return
