...
class MPC_Node:
    ...
    def _trajectory_generator(self, t_now):
        ...
        for i in range(self.T + 1):
            ...
            # 螺旋线
            x = self.spiral_center_x + a * t
            y = self.spiral_center_y + np.sin(a*t + np.pi/2)
            
            # 导数计算用于生成参考状态
            x_dot = a
            y_dot = -a*np.sin(a*t)
            y_ddot = -a*a*np.cos(a*t)
            x_ddot = 0
            
            w = np.arctan2(y_dot, x_dot)
            u_vel = np.sqrt(x_dot**2 + y_dot**2)
            r = (x_dot * y_ddot - y_dot * x_ddot)/(x_dot**2 + y_dot**2) if (x_dot**2 + y_dot**2) > 1e-6 else 0
            
            ref_pos = np.array([x, y, w, u_vel, 0, r])
